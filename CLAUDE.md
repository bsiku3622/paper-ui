...
- **밀도** — 간격(Space) 5단(4px 배수 xs4~xl24)과 컨트롤 크기(ControlSize) 3단(sm·md·lg)을 분리한다. 컨트롤 크기가 바뀌어도 글자는 14 고정(controlFontSize 14/14/14). control 높이 md 34 · table 행 44.
  여백도 컨트롤 전용 축이다 — `controlPaddingX` 10/13/17 을 다섯이 함께 쓴다.
  ⚠ **정사각이 뜻을 갖는 건 Textarea 뿐이다.** 한 줄짜리 컨트롤의 세로는 여백이 아니라 높이
  안의 중앙 정렬이라 맞출 대상이 아니다. `textareaPaddingY` 6.5/9.5/13.5 는 **가로에서** 계산한다
  (가로 − 테두리 1 − 반 줄 3.5). 반대로 세로에서 가로를 유도했다가 물렸다 — 34 에 14 짜리 글자면
  세로로 남는 게 9 뿐이라, 그걸 가로로 옮기면 답답해진다(여러 줄은 줄마다 오른쪽 벽에 닿아 더 심하다)
