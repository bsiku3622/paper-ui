...
- **직교하는 축.** 컴포넌트는 `color`(의미 색) × `variant`(시각 무게)처럼 뜻의 축을 받습니다. 형태는 토큰이 정하고, prop으로 여는 형태 축은 `shape="pill"`처럼 값이 몇 개뿐인 닫힌 어휘뿐입니다. (`status`는 Field 계열의 validation 축으로 따로 있습니다.)
