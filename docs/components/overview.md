...
- **Field** — 한 줄 입력 그룹. 상태는 `status` 축 하나로(invalid boolean 아님), 양옆에 `leading`/`trailing` 어도먼트(아이콘·$·단위)와 `clearable`(×)·`showPasswordToggle`(👁). 쉬는 semantic 테두리는 옅은 `edge`, hover는 `edgeStrong`이라 상태가 기본 입력보다 먼저 소리치지 않습니다. 포커스 때만 바깥에 파란 링이 더해집니다. `shape="pill"`은 Button·Badge와 같은 어휘이고, 검색창이 그 본진입니다 — 반경이 래퍼 하나에만 있고 포커스 링이 `outline`이라 링이 곡선을 저절로 따라갑니다.
