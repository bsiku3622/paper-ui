...
**border — 괘선.** 이름은 하나지만 **두 종류가 섞여 있습니다.**

| | light | dark | 하는 일 |
|---|---|---|---|
| `border.base` | `#e8e8ea` | `#2b2b31` | 면을 나누는 헤어라인 — 카드·구획 |
| `border.strong` | `#d8d8dc` | `#3d3d44` | 또렷한 구획 — table head 밑 |
| `border.control` | `#92929b` | `#5e5e64` | **그 자체가 컨트롤인 선** — 빈 입력칸·꺼진 체크박스·outline |
| `border.controlHover` | `#71717a` | `#79797e` | 그 hover |

앞의 둘은 물러나 있는 게 맞습니다. 이 시스템은 선이 아니라 면으로 나누니까요. 뒤의 둘은 반대입니다 — **빈 Field는 상자 말고 아무 단서가 없고**, 꺼진 Checkbox도 마찬가지입니다. 그 선이 안 보이면 컴포넌트가 안 보입니다.

오래 이 둘이 한 사다리를 쓰고 있었습니다. 그래서 빈 입력칸의 경계가 지면 대비 `1.19`, 체크박스가 `1.39`였고 — 비텍스트 기준(3:1)의 절반도 안 됩니다 — 실제로 데모의 검색창이 헤더에 묻혀 사라졌습니다. `edge`와 `edgeStrong`을 가른 것과 **같은 문제, 같은 해법**입니다: 재는 대상이 다르면 값을 나눕니다. 올려도 화면이 무거워지지 않습니다. 오히려 컨트롤이 앞으로 나오고 구조선은 그대로 뒤에 남아 위계가 또렷해집니다.
...
**중립 컨트롤의 경계는 `border.control`입니다.** outline 버튼의 테두리와 빈 Field의 테두리는 같은 일을 하는 같은 선이라 한 값을 씁니다 — primary가 자기 `edgeStrong`을 들지 않는 이유입니다(Field는 색 family의 소비자가 아니어서, primary 밑에 두면 입력이 "primary 색"을 참조하는 이상한 모양이 됩니다). accent는 반대로 각 색이 자기 `edgeStrong`을 소유합니다. 그쪽 테두리는 라벨 색과 맞아야 하니까요.
...
`Button`·`Field`·`Select`·`Textarea`·`Tabs`가 이 사다리를 공유합니다. **공유하는 것은 height와 글자 크기이고, 가로 여백은 역할로 갈립니다.** 버튼과 탭은 라벨이 상자를 정의하는 자리라 좌우가 넉넉해야 형태가 살고(`controlPaddingX` 10/13/17), 입력류는 상자가 먼저 있고 글자가 그 안에 놓이는 자리라 더 좁게 갑니다(`inputPaddingX` 7/9/12). shadcn(버튼 16 · 입력 12)·Ant(15 · 11)도 같은 방향으로 갈라 둡니다.

`inputPaddingX`는 `(height − 테두리 2 − 글자 14) / 2`입니다. 34짜리 Field는 세로로 이미 9를 갖고 있으므로 — 높이가 그렇게 만듭니다 — 가로에도 9를 주면 **글자가 테두리에서 사방 같은 거리에 앉습니다.** 눈이 읽는 것은 선언한 padding 값이 아니라 잉크의 자리입니다. Textarea의 세로 선언값이 5.5인 것도 그래서인데, 줄상자에 반 줄 여백(3.5)이 이미 들어 있어 5.5 + 3.5 = 9로 같은 자리에 앉습니다. 선언값을 정사각(9/9)으로 맞추면 되레 첫 줄이 왼쪽 여백보다 3.5 아래에서 시작합니다.
