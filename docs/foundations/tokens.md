...
**controlSize** — 컴포넌트 `size` 3단(`sm`·`md`·`lg`), height·padding만 움직임 · **controlPaddingX** 10/13/17 · **textareaPaddingY** 6.5/9.5/13.5 · **controlFontSize** 14/14/14 (밀도와 분리 — 14가 하한이자 상한)

**정사각이 뜻을 갖는 것은 Textarea뿐입니다.** 한 줄짜리 컨트롤의 세로는 여백이 아니라 **높이 안의 중앙 정렬**입니다 — 34짜리 상자에 14짜리 글자가 가운데 앉는 것을 여백으로 읽는 사람은 없으니, 가로를 거기 맞출 이유가 없습니다. 글이 여러 줄로 흐르는 상자만 사방이 다 보입니다.

그래서 `textareaPaddingY`는 **가로에서 세로를 계산합니다** — `controlPaddingX − 테두리 1 − 반 줄 여백 3.5`입니다. 반 줄 여백은 `(줄상자 21 − 글자 14) / 2`로, 줄상자가 글자 위아래에 이미 갖고 있는 몫입니다. md에서 `1 + 9.5 + 3.5 = 14`가 되어 가로(`1 + 13`)와 같은 자리에 앉습니다. 선언값을 정사각(13/13)으로 맞추면 되레 첫 줄이 왼쪽 여백보다 3.5 아래에서 시작합니다 — 눈이 읽는 것은 선언값이 아니라 잉크의 자리입니다.

⚠ **방향이 중요합니다.** 한동안 반대로 세로에서 가로를 유도했습니다(`(height − 2 − 14) / 2 = 9`를 가로에도). 정사각은 됐지만 **기준점이 화면에서 가장 좁은 값**이었습니다 — 34에 14짜리 글자면 세로로 남는 것이 9뿐이니, 그것을 가로로 옮기는 순간 답답해집니다. Textarea는 더 심했습니다: 한 줄짜리 Field는 왼쪽 여백만 보이지만 **글이 넘어가는 상자는 줄마다 오른쪽 벽에 닿습니다.**

`Button`·`Field`·`Select`·`Textarea`·`Tabs`가 이 사다리를 공유합니다 — height, 가로 여백, 글자 크기 전부입니다. Select는 오른쪽만 화살표 자리로 덮고, Textarea는 height를 빼고 세로 여백을 더합니다.
