...
| `border.strong` | `#d8d8dc` | `#3d3d44` | 한 단 또렷하게 — table head 밑·컨트롤 hover |
...
그래서 컨트롤을 더 또렷하게 하려면 컨트롤만 진하게 하는 게 아니라 **이 표를 올립니다.** 카드·구획·입력이 함께 움직이니 회색은 계속 한 벌로 남습니다.

⚠ 강도는 아직 확정이 아닙니다. 지금 값으로 빈 입력칸의 경계는 지면 대비 `1.19`이고, 비텍스트 기준(3:1)에는 한참 못 미칩니다. 사이트를 돌아다니면서 고르려고 `?lab=edge`로 켜지는 도구를 두었습니다.
...
**controlSize** — 컴포넌트 `size` 3단(`sm`·`md`·`lg`), height·padding만 움직임 · **controlPaddingX** 10/13/17(버튼·탭) · **inputPaddingX** 10/12/15(입력칸) · **textareaPaddingY** 6.5/8.5/11.5 · **controlFontSize** 14/14/14 (밀도와 분리 — 14가 하한이자 상한)

**가로 여백은 라벨과 값이 갈립니다.** 버튼과 탭의 글자는 상자를 *정의하는* 라벨이라 좌우가 넉넉해야 형태가 살고(`controlPaddingX` 10/13/17), 입력칸은 상자가 먼저 있고 값이 그 안에 놓이는 자리라 한 단 좁습니다(`inputPaddingX` 10/12/15). shadcn(버튼 16 · 입력 12)·Ant(15 · 11)도 같은 방향으로 갈라 둡니다.

입력칸 값은 **잉크 가로:세로를 1.3**으로 둔 것입니다. 한 줄짜리 입력칸의 세로 잉크는 높이가 정해 버립니다 — `(34 − 14) / 2 = 10`. 가로를 거기 딱 맞추면(정사각) 답답하고, 버튼 사다리까지 벌리면 1.4배라 글자가 눌린 것처럼 보입니다. 글 상자의 좌우 여백이 위아래보다 조금 넉넉한 것은 활자 조판의 오랜 관습이고, 1.2~1.5가 그 밴드입니다.

⚠ **정사각은 높이를 올리지 않는 한 불가능합니다.** 34짜리 상자에 14짜리 글자면 세로로 남는 것이 10뿐이라, 잉크를 사방 14로 맞추려면 높이가 **42**여야 합니다 — 같은 줄의 버튼과 8이 어긋납니다. 버튼까지 42로 올리면 이 시스템이 스스로 정한 밀도(34 anchor, shadcn 36과 Atlassian 32 사이)를 벗어납니다.

**정사각이 뜻을 갖는 것은 Textarea뿐입니다.** 글이 여러 줄로 흐르면 위·아래·좌·우가 다 보이기 때문입니다 — 한 줄짜리 Field는 좌우만 보입니다. 그리고 **여러 줄은 가로 여백이 매 줄 눈에 걸립니다**: 글이 넘어가는 상자는 줄마다 오른쪽 벽에 닿으므로, 같은 값이라도 Field보다 좁게 느껴집니다.

`textareaPaddingY`는 **가로에서 세로를 계산합니다** — `inputPaddingX − 반 줄 여백 3.5`입니다. 반 줄 여백은 `(줄상자 21 − 글자 14) / 2`로, 줄상자가 글자 위아래에 이미 갖고 있는 몫입니다(테두리는 양쪽에 똑같이 붙어 상쇄됩니다). md에서 `1 + 8.5 + 3.5 = 13`이 되어 가로(`1 + 12`)와 같은 자리에 앉습니다. 선언값을 정사각(12/12)으로 맞추면 되레 첫 줄이 왼쪽 여백보다 3.5 아래에서 시작합니다 — 눈이 읽는 것은 선언값이 아니라 잉크의 자리입니다.

⚠ 한동안 반대로 **세로에서 가로를 유도**했습니다(`(34 − 2 − 14) / 2 = 9`를 가로에도). 정사각은 됐지만 기준점이 화면에서 가장 좁은 값이라 눈에 띄게 답답했습니다. 정사각 자체가 목표가 아니라, *가로가 세로보다 조금 넉넉한 것*이 목표입니다.
