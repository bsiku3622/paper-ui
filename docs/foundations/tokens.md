...
**border — 괘선.** 두 값 한 벌이고, **화면의 회색 선은 전부 여기서 나옵니다.**
...
**accent — 4색 × 5자리.** 이름이 곧 의미(`info`·`success`·`warning`·`error`) — API 에 hue 이름(blue 등)은 없습니다. 각 색은 다섯 자리: `solid`(채운 면/점) · `ink`(면 위 글자, AA) · `wash`(옅은 면이자 solid 위 글자) · `edge`(wash의 괘선) · `edgeStrong`(홀로 서는 outline 선).

라이트

| | solid | ink | wash | edge | edgeStrong |
|---|---|---|---|---|---|
| **info** | `#2563eb` | `#1d4ed8` | `#f5f9fe` | `#d1e1f5` | `#5694e9` |
| **success** | `#15803d` | `#15803d` | `#f5fdf8` | `#cff0dc` | `#35a75f` |
| **warning** | `#b45309` | `#b45309` | `#fefbf3` | `#f4e7b5` | `#c5861b` |
| **error** | `#dc2626` | `#b91c1c` | `#fdf7f7` | `#f4d8d8` | `#ed6969` |
...
| | solid | ink | wash | edge | edgeStrong |
|---|---|---|---|---|---|
| **info** | `#5492ec` | `#8ebcff` | `#0d1c32` | `#1a355d` | `#4c6fa3` |
| **success** | `#32ac64` | `#81cf99` | `#062211` | `#094021` | `#3a7b52` |
| **warning** | `#c58300` | `#e4b066` | `#291800` | `#4c2e00` | `#8e672c` |
| **error** | `#df6768` | `#f99e9b` | `#2f1211` | `#572223` | `#a15a59` |

톤별 자리(다크, OKLCH `L / C`) — `wash` `.225/.048` · `edge` `.330/.078` · `solid` `.660/.150` · `ink` `.790/.110`.
...
⚠ `edge`만 접기에서 벗어납니다(엄밀한 거울은 L `.305`). 거기서는 canvas 위 `1.42`로 `border.base`(`1.35`)와 거의 같아 **outline 버튼의 테두리가 보이지 않습니다.** `.330`으로 올려 `1.50~1.60`을 줍니다 — 대신 `soft`의 hover 폭이 ΔL `.104`로 라이트(`.073`)보다 넓습니다. **`edge`가 두 일(테두리 · hover 면)을 겸하는 구조 문제**였고, `edgeStrong`을 신설해 갈랐습니다.

다크 계약 (전 항목 통과) — `solid`↔`well` `4.56~5.25` (비텍스트 3:1) · `wash`↔`solid` `5.16~5.80` (AA 4.5) · `ink`↔`wash` `8.55~9.09` · `ink`↔`raised` `9.60~10.46`.

⚠ **채운 면 위 글자는 `wash`입니다.** `paper.raised`는 다크에서 최암이라 쓸 수 없고, 별도 `solidFg`는 wash와 거의 같은 끝점을 중복하면서 라이트 대비가 `3.91~4.21`로 약했습니다. Veil wash를 재사용하면 라이트 `4.56~4.89`, 다크 `5.16~5.80`으로 양쪽 모두 AA를 넘습니다.

**accent 5자리** — `solid`(채운 면) · `ink`(지면 위 글자, AA) · `wash`(옅은 면이자 solid 위 글자) · `edge`(wash의 괘선, 그리고 `soft`의 hover 면) · `edgeStrong`(**홀로 서는 테두리** — `outline` 변형).

**중립 outline의 테두리는 `border.base`입니다** — 카드 헤어라인·빈 Field와 같은 선입니다. primary가 자기 `edgeStrong`을 들지 않는 이유이고, accent는 반대로 각 색이 자기 것을 소유합니다.
...
**controlSize** — 컴포넌트 `size` 3단(`sm`·`md`·`lg`), height·padding만 움직임 · **controlPaddingX** 10/13/17(버튼·탭) · **inputPaddingX** 7/9/12(입력칸) · **textareaPaddingY** 3.5/5.5/8.5 · **controlFontSize** 14/14/14 (밀도와 분리 — 14가 하한이자 상한)

**가로 여백은 라벨과 값이 갈립니다.** 버튼과 탭의 글자는 상자를 *정의하는* 라벨이라 좌우가 넉넉해야 형태가 살고(`controlPaddingX` 10/13/17), 입력칸은 상자가 먼저 있고 값이 그 안에 놓이는 자리라 정사각 geometry를 씁니다(`inputPaddingX` 7/9/12).

입력칸 값은 **보이는 잉크가 정사각**입니다. md 한 줄 입력의 세로 거리는 `(34 − 14) / 2 = 10`, 가로는 `border 1 + padding 9 = 10`입니다. 한때 답답함을 피하려 10/12/15까지 벌렸지만, 아이콘과 chevron이 들어가면 좌우 비대칭이 더 크게 읽혔습니다. 높이를 키우지 않고 Field·Select·Textarea의 공통 x padding을 줄이는 쪽으로 확정했습니다.

`textareaPaddingY`는 **가로에서 세로를 계산합니다** — `inputPaddingX − 반 줄 여백 3.5`입니다. 반 줄 여백은 `(줄상자 21 − 글자 14) / 2`로, 줄상자가 글자 위아래에 이미 갖고 있는 몫입니다. md에서 `1 + 5.5 + 3.5 = 10`이 되어 가로(`1 + 9`)와 같은 자리에 앉습니다. 눈이 읽는 것은 선언값이 아니라 잉크의 자리입니다.

`Button`·`Field`·`Select`·`Textarea`·`Tabs`가 이 사다리를 공유합니다 — height와 글자 크기입니다. Select는 오른쪽을 화살표 자리로 한 번 더 덮고, Textarea는 height를 빼고 세로 여백을 더합니다.

Link는 여기 없습니다 — 상자가 아니라 글자라, 높이도 여백도 없이 주변 텍스트에 실려 갑니다.

**베이스에서 벗어나려면 통로로 나갑니다.** `internal/sizeLadder`의 `ladderRules(patch)`가 그 자리입니다 — 인자 없이 부르면 베이스 그대로(Button·Tabs), 키를 돌려주면 그 축만 교체하고(Field·Select), `null`을 돌려주면 그 축을 뺍니다(Textarea의 `height`). 사다리를 손으로 베끼면 언젠가 갈라지고(Select가 실제로 그랬습니다), 베이스가 전부를 강제하면 역할이 다른 컨트롤이 억지로 같아집니다. 통로로 나가면 무엇을 왜 바꿨는지가 호출부 한 줄에 남고, 나머지 축은 베이스가 바뀔 때 저절로 따라옵니다.

**badge** — 높이 20/22/24, 가로 여백 `badgePaddingX` 6/8/10, **글자는 12 고정**입니다. 가로 여백이 높이와 같은 보폭(Δ2)으로 움직여 세 크기가 한 실루엣의 축소·확대로 읽힙니다. 배지는 컨트롤이 아니라 자기 축을 듭니다 — 컨트롤의 가로 여백은 세로 여백에 대한 비로 잡히는데, 배지는 20~24 상자에 18짜리 줄상자가 들어가 나눌 세로가 0~2밖에 없습니다. 배지도 컨트롤과 같은 규칙을 집니다. 밀도와 가독성은 다른 축이라, 배지가 작아진다고 글자가 따라 내려가지 않습니다. 컨트롤이 14인 자리에서 배지가 12인 것은 배지가 버튼 라벨보다 한 단 낮은 표식이기 때문이고, 각 tier가 자기 앵커를 하나씩 듭니다.
...
토큰의 이름은 `radius.full`이지만 컴포넌트가 여는 prop은 `shape="pill"`입니다. 이름이 갈리는 데는 이유가 있습니다. Box는 곡선의 크기를 고릅니다 — `sm`·`md`·`lg`·`full`이 한 사다리 위에 있고, `radius`는 그중 어느 칸이냐를 묻습니다. Button·Badge·Field·Select·Tabs는 사다리를 아예 열지 않습니다. 물어보는 것은 실루엣이 알약이냐 아니냐 하나뿐이라, `radius`라는 이름은 있지도 않은 선택지를 암시합니다. 같은 999px을 두 이름으로 부르는 게 아니라, 서로 다른 두 질문에 각자의 이름을 준 것입니다.
