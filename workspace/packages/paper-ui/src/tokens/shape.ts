...
//
// **세로 잉크 + 3.** 한 줄짜리 입력칸의 세로 잉크는 높이가 정해 버린다((height − 글자
// 14) / 2 = 8 · 10 · 13). 가로를 거기에 딱 맞추면(정사각) 답답하고, 버튼 사다리
// (controlPaddingX 10 · 13 · 17)까지 벌리면 세로 대비 1.4 배라 글자가 눌린 것처럼 보인다.
// 그 사이에서 잉크 가로:세로를 **1.3** 으로 둔다 — 글 상자의 좌우 여백이 위아래보다
// 조금 넉넉한 건 활자 조판의 오랜 관습이고, 1.2~1.5 가 그 밴드다.
//
//   잉크 세로 8 · 10 · 13  →  잉크 가로 11 · 13 · 16  →  여백 10 · 12 · 15
//
...
//
// ⚠ **정사각은 높이를 안 올리는 한 불가능하다.** 34 짜리 상자에 14 짜리 글자면 세로로
// 남는 게 10 뿐이라, 잉크를 사방 14 로 맞추려면 높이가 **42** 여야 한다 — 같은 줄의
// 버튼과 8 이 어긋난다. 버튼까지 42 로 올리면 이 시스템이 스스로 정한 밀도(34 anchor,
// shadcn 36 과 Atlassian 32 사이)를 벗어난다. 그래서 1.3 에서 멈춘다.
//
// ⚠ 한때 반대로 **세로에서 가로를 유도**했다 — (34 − 2 − 14) / 2 = 9 를 가로에도 줬다.
// 정사각은 됐는데 기준점이 화면에서 가장 좁은 값이라 눈에 띄게 답답했다. 정사각 자체가
// 목표가 아니라, *가로가 세로보다 조금 넉넉한 것* 이 목표다.

export const INPUT_PADDING_X = {
  sm: REM(10), // 잉크 11 (세로 8)
  md: REM(12), // 잉크 13 (세로 10) ◀ anchor
  lg: REM(15), // 잉크 16 (세로 13)
} as const;
...
//
// md 기준 1 + 8.5 + 3.5 = 13 = 1 + 12. 선언값을 정사각(12 / 12)으로 맞추면 되레 첫 줄이
