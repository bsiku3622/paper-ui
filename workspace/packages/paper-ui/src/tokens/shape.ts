...
//
// **가로에서 세로를 계산한다.** Textarea 의 잉크가 테두리에서 사방 같은 거리에 앉도록,
// 가로 여백(controlPaddingX)에서 테두리와 반 줄 여백을 빼고 남긴 값이다:
//
//   paddingBlock = controlPaddingX − 테두리 1 − 반 줄 여백 3.5
//   (반 줄 여백 = (줄상자 21 − 글자 14) / 2. 줄상자가 글자 위아래로 이미 갖고 있는 몫이다.)
//
// ⚠ **방향이 중요하다.** 한때 반대로 세로에서 가로를 유도했다 — 높이 34 에서
// (34 − 2 − 14) / 2 = 9 를 뽑아 가로에도 9 를 줬다. 정사각은 됐는데 **기준점이 화면에서
// 가장 좁은 값**이었다: 34 짜리 상자에 14 짜리 글자면 세로로 남는 게 9 뿐이라, 그걸 가로로
// 옮기는 순간 답답해진다. 게다가 Textarea 는 높이가 안 정해져 있어 애초에 높이에서 유도할
// 근거도 없었는데 Field 의 값을 그대로 물려받았다.
//
// ⚠ **여러 줄은 가로 여백이 매 줄 눈에 걸린다.** 한 줄짜리 Field 는 왼쪽 여백만 보이지만,
// 글이 넘어가는 상자는 줄마다 오른쪽 벽에 닿는다 — 같은 9 라도 Textarea 가 훨씬 답답한 이유다.
//
// Field 는 이 표를 안 쓴다. 거기 세로는 여백이 아니라 **높이 안의 중앙 정렬**이라 계산할
// 게 없고, 한 줄짜리 값이 상자 가운데 앉는 걸 여백으로 읽는 사람도 없다.

export const TEXTAREA_PADDING_Y = {
  sm: REM(6.5), //  10 − 1 − 3.5
  md: REM(9.5), //  13 − 1 − 3.5 ◀ anchor
  lg: REM(13.5), // 17 − 1 − 3.5
} as const;
