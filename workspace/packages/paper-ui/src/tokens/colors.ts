...
//
// ⚠ **불변식 C — 채움(solid)도 뒤집힌다. 라이트를 canvas 축으로 접은 값이다.**
//
//     라이트  canvas L .991 → solid .527~.577 (아래로 평균 .440) · solidFg .92~.94
//     다크    canvas L .205 → solid .645      (위로   .440)      · solidFg .245
//
// 오래 다크 solid 를 L .440 으로 **눌러** 뒀다. 이유는 그럴싸했다 — 옅은 글자(solidFg
// .92~.94)를 얹어야 하니 면이 어두워야 한다는 것. 그런데 그러면 **채움이 면에서 안
// 떨어진다**: solid→면 대비가 1.79~2.45 였다(라이트는 3.74~5.17). 비텍스트 3:1 을 못
// 넘기니 배지·버튼의 *윤곽 자체* 가 계약을 깬 것이고, 글자 없는 상태 점
// (`pui-{accent}-dot`)은 아예 안 보였다.
//
// **눌러서는 못 고친다 — 갈 수 있는 방향이 하나뿐이다.** 채움이 3:1 을 넘기려면 가장
// 밝은 면(well #292929, 상대휘도 .0222)에서 그만큼 떨어져야 하는데, *아래로* 가려면
// 채움의 상대휘도가 .0722/3 − .05 = −.0019 여야 한다 — 음수라 존재하지 않는다. 이
// 팔레트의 다크 면이 L .196~.281 에 몰려 있어 아래에 쓸 자리가 없기 때문이다. 답은
// primary 가 이미 하던 것이다(PRIMARY_DARK: 흰 채움 / 검정 글자) — **밝은 채움 +
// 어두운 글자.** accent solid 만 안 뒤집힌 축으로 남아 있었다.
//
// 뒤집고 나면 세 가지가 한꺼번에 맞는다:
//     solid→면      4.20~5.84  (라이트 3.74~5.17)   ← 3:1 계약 회복
//     solidFg→solid 4.76~5.13  (라이트 3.91~4.21)
//     Banner        4.76~5.13  (예전 2.24~2.45)     ← 아래 참조
//
// ⚠ **Banner 가 조용히 깨져 있었다.** 채운 면 위 글자를 `paper.raised` 로 하드코딩하고
// 있었는데("종이색" 이라는 뜻이었다), 다크의 raised 는 최암(#151515)이다. 그래서 status
// Banner 의 글자가 다크에서 **2.24~2.45** 였다 — 명백한 AA 실패. 지금은 Banner 도
// solidFg 를 쓴다. 채운 면 위 글자의 정본은 처음부터 그 자리였다.
//
// ⚠ solidFg 는 **더 이상 라이트와 공유하지 않는다.** "채움 위 글자는 ground 와 무관하다"
// 는 옛 근거는 채움이 양 테마에서 같은 방향일 때만 참이었다.
//
// ⚠ 접기를 그대로 따르면 solidFg 는 .265 여야 한다(라이트의 solid→solidFg 거리 평균
// .378 을 뒤집은 값). 그런데 그 자리에서 error 가 4.49 로 AA 를 **.01 차이로** 놓친다.
// 그래서 .245 까지 내려 잡았다 — 거울에서 벗어난 유일한 자리이고, 벗어난 이유가 이것이다.
//
// ⚠ **라이트 solidFg 는 아직 AA 미달이다** — 옅은 글자가 진한 채움 위에서 3.91~4.21 로
// 4.5 를 못 넘는다(error 3.91 이 최악). 다크와 달리 라이트는 채움을 더 진하게 내리면
// 풀리므로 구조가 아니라 값의 문제다. 이번 뒤집기 범위 밖이라 손대지 않고 남겨 둔다.
//
...
// solid 였으니 구조도 이쪽이 맞다.
//
// wash 는 **사다리 밖(well 너머)** 이어야 한다 — 면 위에 놓인 배지가 그 면과 같은 밝기면
// 사라지기 때문이다. 사다리를 뒤집기 전에는 이 조건을 "raised 보다 위" 라고 적어 뒀는데,
// 지금 raised 는 최암이라 그 문장은 저절로 참이 돼 아무것도 막지 못한다. 실제로 막아야
// 하는 건 최명인 **well(#292929)** 이다 — 지금 wash 는 L .365~.377 로 그 너머에 있다.
//
// ink(글자)도 L .745 로 올렸다. 면이 밝아진 만큼 따라 오르지 않으면 raised 위에서
// info 가 4.46 으로 AA 를 놓친다.
const ACCENT_DARK = {
  info: { solid: "#5d90cf", solidFg: "#132133", ink: "#88afe2", wash: "#334051", edge: "#3c516b" },
  success: { solid: "#4aa274", solidFg: "#0d2619", ink: "#81bc9a", wash: "#33473b", edge: "#3b5a48" },
  warning: { solid: "#ae8734", solidFg: "#2a1e08", ink: "#c6a86e", wash: "#483e2b", edge: "#5d4d2f" },
  error: { solid: "#c87173", solidFg: "#311819", ink: "#e09595", wash: "#503736", edge: "#694242" },
} as const;
