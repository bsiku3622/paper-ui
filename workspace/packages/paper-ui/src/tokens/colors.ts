...
// ───── border — 괘선 ───────────────────────────────────────────────────────
//
// 이 시스템의 구획은 얇은 뉴트럴 선으로 나뉜다 (shadcn/Atlassian 의 결). 다만 **모든
// 선이 같은 일을 하는 건 아니다** — 두 종류가 섞여 있다.
//
//   면을 나누는 선 (base · strong)  카드 헤어라인 · 구획 · table head 밑.
//     얇을수록 좋다. 이 시스템은 선이 아니라 면으로 나누므로, 선은 물러나 있어야 한다.
//   그 자체가 컨트롤인 선 (control)  빈 입력칸 · 체크박스 · outline 버튼.
//     여기서 선은 장식이 아니라 형태다. 빈 Field 는 상자 말고 아무 단서가 없다.
//
// ⚠ 오래 이 둘이 한 사다리를 쓰고 있었다. 그래서 빈 입력칸의 경계가 지면 대비 **1.19**,
// 체크박스가 1.39 였다 — 비텍스트 기준(3:1)의 절반도 안 되고, 실제로 데모의 검색창이
// 헤더에 묻혀 사라졌다. accent 의 edge / edgeStrong 을 가른 것과 **같은 문제, 같은 해법**
// 이다: 재는 대상이 다르면 값을 나눈다.
//
// control 을 3:1 로 올려도 화면이 무거워지지 않는다 — 오히려 컨트롤이 앞으로 나오고
// 구조선(base)은 그대로 뒤에 남아 위계가 또렷해진다(실물 비교로 확인).

export const BORDER = {
  base: "#e8e8ea", // 면을 나누는 헤어라인 — 거의 안 보이게. 선보다 면이 먼저.
  strong: "#d8d8dc", // 또렷한 구획 — 섹션 · table head 밑처럼 선이 필요한 자리
  //
  // ⚠ **강도는 아직 확정이 아니다.** 한때 3:1(#92929b)까지 올렸다가 한 단 내렸다 —
  // 백재원이 "input·select 가 이상하고 버튼은 괜찮다" 고 했다. 같은 값인데 자리마다 달리
  // 읽히는 데는 이유가 있다: outline 버튼은 진한 라벨이 테두리와 균형을 잡는데, **빈
  // 입력칸은 안이 비어 있어 테두리만 소리친다.** 게다가 입력칸이 훨씬 길어 선의 총량이 많다.
  // 지금 값은 "전부 한 단 낮춘" 안이고, 안 맞으면 (a) 버튼용 변형을 따로 두거나
  // (b) lab 으로 강도를 훑어 고른다. 되돌릴 자리는 이 두 줄뿐이다.
  control: "#b7b7bc", // 그 자체가 컨트롤인 선 (흰 면 위 1.97:1) — 입력 · 체크박스 · outline
  controlHover: "#94949b", // 그 hover (3.02:1). 쉬는 상태보다 한 단은 확실히 진해야 한다
} as const;
...
const BORDER_DARK = {
  base: "#2b2b31", //   헤어라인 — well 바로 너머, GRAY 쪽 (raised 위 1.38:1)
  strong: "#3d3d44", // 또렷한 구획 — table head·섹션 (1.80:1)
  // 컨트롤의 경계 — 입력 면(raised) 위에서 3:1. 다크의 raised 는 **최암**이라 이 값이
  // 라이트보다 밝은 쪽으로 간다(사다리가 뒤집히는 것과 같은 이유).
  // 라이트와 같은 보폭으로 한 단씩 내렸다(위 ⚠ 참고). 다크 hover 가 예전 control 값이다.
  control: "#44444a", //      raised 1.96 · canvas 1.96
  controlHover: "#5e5e64", // raised 3.01
} as const;
