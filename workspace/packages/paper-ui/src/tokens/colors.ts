...
// ⚠ **불변식 — 사다리의 순서는 양 테마가 같다.** well < sunken < canvas < raised.
...
const PAPER_DARK = {
  raised: "#212121", // 떠오른 면 = 최명 (카드 · 입력 · 모달)
  canvas: "#171717", // 지면
  sunken: "#131313", // 얕은 홈 — 사이드바 · table head
  well: "#0e0e0e", //   가장 깊은 홈 — 트랙 · soft
} as const;
...
// ⚠ raised 가 내려온 만큼 헤어라인을 **눌렀다** — #3c3c3c 를 그대로 두면 raised 위
// 1.46 이 되어, 밝은 테두리가 어두운 면을 outline 해 카드를 오려붙인 판으로 만든다
// (실제 다크 UI 의 카드 테두리는 패널 대비 1.18~1.42 — Linear 1.18 · Notion 1.22 ·
// GitHub 1.42). 값을 눌러도 **역할은 오히려 커졌다**: canvas↔raised 가 1.11 뿐이라
// 카드를 지면에서 가르는 일을 이제 면이 아니라 이 선이 한다.
const BORDER_DARK = {
  base: "#383838", //   헤어라인 (raised 위 1.37:1) — 카드를 세우는 주역
  strong: "#585858", // 강한 경계 — table head·섹션·컨트롤 테두리 (2.26:1)
} as const;
