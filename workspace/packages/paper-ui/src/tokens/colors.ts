...
//
// 다크는 아래로 좁고 위로 넓다: canvas 아래 두 칸은 .021 · .019 로 촘촘히 두고
// (well 이 L .165 — 예전 .145 처럼 순검정에 잠기지 않는 선), raised 만 .065 로 크게
// 띄운다. 어두운 지면에서 카드가 뜨려면 라이트의 .0167 로는 어림없다.
//
// 이 배치가 라이트가 만족하던 두 관계를 그대로 세운다:
//     사이드바(sunken) ↔ 본문(canvas)   인접   1.04  ← 은은
//     Tabs 트랙(well) ↔ 활성(raised)     세 칸  1.28  ← 확실
const PAPER_DARK = {
  raised: "#262626", // 떠오른 면 = 최명 (카드 · 입력 · 모달)
  canvas: "#171717", // 지면
