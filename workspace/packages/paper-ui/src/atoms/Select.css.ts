...
export const selectRoot = style({
  ...
  selectors: {
    "&:hover:not(:focus)": { borderColor: tokens.color.primary.base },
...
// size 3 단 — 가로는 Field 와 같은 입력칸 여백으로 갈아 끼우고, 오른쪽만 화살표 자리로
// 한 번 더 덮는다. Select 는 값을 *고르는* 상자라 값을 *쓰는* 상자와 같은 열에 서야 한다.
//
