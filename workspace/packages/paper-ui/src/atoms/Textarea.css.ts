...
// 가로와 글자는 베이스 그대로다.
//
// **잉크가 사방 같은 거리에 앉는다.** 세로 선언값(9.5)이 가로(13)보다 작은 건 재는 대상이
// 달라서다 — 줄상자에 이미 반 줄 여백 3.5 가 들어 있어, 1 + 9.5 + 3.5 = 14 로 가로(1 + 13)와
// 같은 자리가 된다. 선언값을 정사각으로 맞추면 되레 첫 줄이 왼쪽 여백보다 3.5 내려간다 —
// 눈이 읽는 건 선언값이 아니라 잉크의 자리다.
//
// ⚠ 기준은 **가로** 다. 세로에서 가로를 유도하던 시절엔 (height − 2 − 14) / 2 = 9 가 나와
// 가로가 9 로 좁아졌고, 여러 줄이 줄마다 오른쪽 벽에 닿아 눈에 띄게 답답했다.
