      ...
      <Stack gap="xl" style={{ maxWidth: "48rem" }}>
        ...
        <Section id="sec-button" title="Button">
          <Spec label="variant — 시각 무게" testid="spec-button-variants">
            ...
            <Button variant="hairline" data-testid="btn-hairline">hairline</Button>
        ...
        <Section id="sec-form" title="Field · Label · Select · Checkbox">
          ...
          <Spec label="base — 넷이 높이·가로 여백·글자를 공유한다 (md 13 · 14)" testid="spec-control-base">
        ...
        <Section id="sec-feedback" title="Alert · outline">
          ...
          <Spec label="Alert — variant (같은 색, 네 가지 면의 무게)" testid="spec-alert-variant">
            <Inline gap="md" wrap align="start">
              {(["soft", "solid", "outline", "hairline", "quiet"] as const).map((v) => (
          ...
          {/* 테두리만 있는 변형이 둘이다. 세기가 아니라 **어느 선이냐**로 갈린다 —
              outline 은 컨트롤이 소유한 선(color family 의 edgeStrong), hairline 은 지면이
              소유한 선(카드 헤어라인·빈 입력칸과 같은 border.base). 그래서 hairline 은 여러
          ...
          <Spec label="hairline — 지면이 소유한 선 (카드·빈 입력칸과 같은 헤어라인)" testid="spec-hairline">
            <Inline gap="sm" wrap align="center">
              <Button variant="hairline" data-testid="hairline-primary">기본</Button>
              {STATUSES.map((c) => (
                <Button key={c} color={c} variant="hairline" data-testid={`hairline-${c}`}>{c}</Button>
              ))}
              {STATUSES.map((c) => (
                <Badge key={c} color={c} variant="hairline">{c}</Badge>
              ))}
              <Box style={{ width: "9rem" }}>
                <Field placeholder="같은 선" aria-label="같은 선" data-testid="hairline-field" />
              </Box>
            </Inline>
          </Spec>
        </Section>
