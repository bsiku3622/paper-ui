...
export const Tokens = () => {
  ...
  return (
    <PlaygroundLayout active="tokens">
      ...
      <Stack gap="xl">
        {/* ── Color · 흰과 검정 ─────────────────────────────── */}
        <Section id="tok-neutral" title="Color · surface (면 깊이) · ink · border · primary" desc="surface 는 Box 의 깊이 축(색 아님) — raised(최명)·canvas(기준면)·sunken·well. 토큰은 color.paper.*. 다크는 YouTube·ChatGPT 결의 깊은 중립 base. ⚠ **다크 사다리는 라이트의 거울이다 — 순서가 반대다.** 각 테마에는 목표가 있고 raised 가 거기 가장 가깝다: 라이트는 WHITE|raised|canvas|sunken|well|GRAY, 다크는 BLACK|raised|canvas|sunken|well|GRAY. 그래서 **다크에서는 raised 가 최암, well 이 최명**이다. 간격은 canvas 를 축으로 라이트를 접은 값이라 쌍의 거리가 양 테마에서 거의 같다(canvas↔raised 1.019 vs 1.026). primary(검정 일꾼, 면 채우는 유일한 색)·ink(마크)·border·focus·scrim 은 잉크·경계·오버레이. ⚠ **border 는 두 종류가 섞여 있다** — base·strong 은 *면을 나누는 선*(카드 헤어라인·구획)이라 물러나 있는 게 맞고, control·controlHover 는 *그 자체가 컨트롤인 선*(빈 입력칸·꺼진 체크박스·outline 버튼)이라 자기 면 위에서 3:1 을 진다. 오래 한 사다리를 쓰다 보니 빈 입력칸의 경계가 1.19 였고, 데모의 검색창이 헤더에 묻혀 사라졌다. accent 의 edge / edgeStrong 을 가른 것과 같은 문제·같은 해법이다. 값은 현재 테마로 표시된다.">
          <Inline gap="lg" wrap>
